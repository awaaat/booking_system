...
from django.db.models import QuerySet, Avg, ObjectDoesNotExist
...
class ListingService:
    ...
    def create_listing(user: CustomUser, data: dict) -> Listing:
        ...
        # Extract and validate geospatial location
        ...
        if longitude is not None and latitude is not None:
            ...
            raise RestValidationError("Longitude and latitude are required for location")
    ...
    def update_listing(user: CustomUser, listing: Listing, data: dict) -> Listing:
        ...
        if 'property_images' in data:
            ...
            if data.get('clear_images', False):
                listing.property_images.all().delete()
            for image in image_data:
                PropertyImage.objects.create(
                    listing=listing,
                    image=image.get('image'),
